import numpy as np, pandas as pd, matplotlib.pyplot as plt, io, re, json, sys
...
SEED              = 42
# Composite score weights (tweakable, must sum to 1.0)
...
def zscore(series):
    ...
    if not np.isfinite(sd) or sd < 1e-12:
        return (x - mu)  # all zeros
    return (x - mu) / sd

# Map normalized names → common raw axis labels (if present)
RAW_NAME_MAP = {
...
# Attach labels if not present (1=Single, 0=Multi) for weak supervision & evaluation
single_phase_lit = [
    "(CrHfMoNbTaTiVZr)C8",
    "(CrHfNbTaTiVWZr)C8",
    "(CrHfNbTaTi)C5",
...
norm_cols = [c for c in norm_cols if c.lower() not in {"norm_efa","norm_predicted_efa","norm_mdri"}]
if len(norm_cols) >= 3:
    ...
    for cov in ["full", "tied", "diag", "spherical"]:
        try:
            ...
            if (best is None) or (bic < best["bic"]):
                best = {"gmm": g, "bic": bic, "k": k, "cov": cov}
        except Exception:
            pass
...
if is_labeled.sum() >= 1 and len(np.unique(y_lab)) == 2:
    for j in range(resp.shape[1]):
        ...
        diag.append({"comp": j, "winner": winner, "mass_S": float(S), "mass_M": float(M),
                     "purity": float(purity) if purity==purity else None,
                     "mass_ratio": float(ratio) if np.isfinite(ratio) else None})
    diag_df = pd.DataFrame(diag).sort_values("purity", ascending=False)
...
# Sharpness of responsibilities (1 = very sharp, ~1/k = very diffuse)
...
if is_labeled.sum() >= 1 and len(np.unique(y_lab)) == 2:
    single_w = R_lab[y_lab==1].sum(axis=0)
    multi_w  = R_lab[y_lab==0].sum(axis=0)
    comp_is_single = (single_w >= multi_w)
else:
    # Fallback: split GMM means unsupervised (PC1 median)
    means = gmm.means_
...
if is_labeled.sum() >= 1 and len(np.unique(y_lab)) == 2:
    ...
    # save metrics + add sharpness summary
...
# Deduplicate by plot feature
seen = set(); plot_feats_unique = []
for a,b in plot_feats:
    if b not in seen:
        plot_feats_unique.append((a,b))
        seen.add(b)

bands = []  # per-feature bin stats & favorable flags
for (clust_col, feat_col) in plot_feats_unique:
    x = pd.to_numeric(dfX[feat_col], errors="coerce")
    p = dfX["P_single_cal"].astype(float)
    ok = ~(x.isna() | p.isna())
    x, p = x[ok], p[ok]

    q = safe_qcut(x, q=N_BINS_PER_FEAT)
    ...
    # Favorable bins: top-q by bin-mean posterior (for shading / hits)
    ...
    ax.set_ylim(0,1)
    plt.tight_layout(); plt.savefig(OUT/f"posterior_vs_{feat_col.replace(' ','_')}.png", dpi=240); plt.close(fig)
...
# Fast lookup: feature value → bin (via IntervalIndex)
band_lookup = {}
...
# Detect predicted-EFA column (optional)
efa_candidates = [
    "EFA", "efa", "EFA_HEC", "EFA_predicted",
    "Pred_EFA", "pred_efa", "EFA_pred",
    "Predicted_EFA", "Predicted EFA",
    "predicted_efa", "predicted efa",
    "GNN_EFA", "EFA_GNN", "EFA (predicted)"
]
efa_col = find_col(efa_candidates, dfX.columns)
if efa_col is not None:
    print(f"\nUsing '{efa_col}' as predicted EFA column.")
else:
    print("\nNo EFA-like column found; Predicted_EFA / EFA will be NaN in outputs.")
...
for idx, r in dfX.iterrows():
    ...
    # Pull predicted EFA value if column is found
    if efa_col is not None:
        efa_val = pd.to_numeric(r.get(efa_col, np.nan), errors="coerce")
    else:
        efa_val = np.nan
    ...
    rows.append({
        ...
        "hits_count": int(feats_hit),
        "Predicted_EFA": float(efa_val) if np.isfinite(efa_val) else np.nan,
        **{f"hit_{k}": v for k,v in per_feat_hits.items()}
    })

score_df = pd.DataFrame(rows)

# Coverage (fraction of features used for each composition) – for all compositions
n_total_feats = max(1, len(plot_feats_unique))
score_df["coverage"] = score_df["n_feat_used"] / float(n_total_feats)

# Restrict to HEC9 pool
pool9 = score_df[score_df["n_metals"] == float(TARGET_N_METALS)].copy()

# Build composite Score (z-scored components)
...
# Save per-composition summary CSV for *all* orders
summary_cols = ["Composition", "n_metals", "Predicted_EFA", "cum_pmean", "coverage", "P_single_cal"]
summary_df = score_df[summary_cols].copy()

# Order = number of metals (integer)
summary_df["Order"] = summary_df["n_metals"].astype(int)
summary_df = summary_df.drop(columns=["n_metals"])

# Match your requested naming: cum_p_mean, and rename Predicted_EFA -> EFA
summary_df = summary_df.rename(columns={
    "cum_pmean": "cum_p_mean",
    "Predicted_EFA": "EFA"
})

summary_path = OUT / "all_compositions_EFA_posterior_summary.csv"
summary_df.to_csv(summary_path, index=False)
print(f"Saved per-composition summary (all orders) → {summary_path}")
...
# Warn about any missing due to NA filtering or name mismatches
...
for (clust_col, feat_col) in plot_feats_unique:
    ...
    ax.set_title(f"P(single) vs {feat_col}", fontsize=10)
    ax.set_ylim(0,1)
    plt.tight_layout(); plt.savefig(OUT/f"posterior_vs_{feat_col.replace(' ','_')}_with_selected.png", dpi=240); plt.close(fig)
